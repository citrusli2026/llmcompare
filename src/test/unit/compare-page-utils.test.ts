...
// Duplicate the utility functions from compare-client.tsx for isolated testing
...
describe("isBestValue", () => {
  describe("higher is better", () => {
    it("returns true for highest value", () => {
      expect(isBestValue(90, [50, 70, 90], true)).toBe(true);
    });

    it("returns false for non-highest", () => {
      expect(isBestValue(50, [50, 70, 90], true)).toBe(false);
    });

    it("handles tie by returning true (equal highest)", () => {
      expect(isBestValue(90, [80, 90, 90], true)).toBe(true);
    });
  });

  describe("lower is better (price)", () => {
    it("returns true for lowest value", () => {
      expect(isBestValue(5, [5, 10, 15], false)).toBe(true);
    });

    it("returns false for non-lowest", () => {
      expect(isBestValue(10, [5, 10, 15], false)).toBe(false);
    });
  });

  describe("edge cases", () => {
    it("returns false when val is null", () => {
      expect(isBestValue(null, [50, 70, 90], true)).toBe(false);
    });
